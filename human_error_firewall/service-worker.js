...
function calculateRisk(content, context) {
  ...
  // 2. Context Analysis
  // If the target is a password field, increase risk
  if (context.inputType === 'password') {
    sensitivityScore += 2;
    triggeredCategories.push("Password Field");
  }

  // Domain whitelist check (Conceptual - could be expanded)
  // If domain is known internal/safe, maybe lower risk? 
  // For now, we assume all public domains are potentially risky if data is sensitive.
